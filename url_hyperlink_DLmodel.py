...
class PhishingDataset(Dataset):
    def __init__(self, dataframe, char_cols, heuristic_cols, scaler=None):
        ...
        # BOTH URL-lexical and Hyperlink features
...
class HybridModel(nn.Module):
    """
    Two-headed model:CNN head: char sequence , FC head :ALL heuristic features (URL-Lexical + Hyperlink)
    """
    def __init__(self, vocab_size, embed_dim, seq_len, heuristic_in_features,
                 conv_out_features=64, heuristic_out_features=32, padding_idx=0): 
        ...
        flat_size_after_pool1 = (seq_len + 2*1 - 3) // 1 + 1 
        flat_size_after_pool1 = (flat_size_after_pool1 - 2) // 2 + 1 
        flat_size_after_pool2 = (flat_size_after_pool1 + 2*1 - 3) // 1 + 1 
        flat_size_after_pool2 = (flat_size_after_pool2 - 2) // 2 + 1 
        flat_size = 128 * flat_size_after_pool2

        if seq_len == 200:
             flat_size = 128 * (seq_len // 4)
        else:
             print(f"[Warning] seq_len is {seq_len}, calculated flat_size is {flat_size}. Using dynamic calculation.")
        ...
        #2.Heuristic head
        self.heuristic_fc1 = nn.Linear(heuristic_in_features, 64) 
        self.heuristic_relu1 = nn.ReLU()
        self.heuristic_fc2 = nn.Linear(64, heuristic_out_features)
        self.heuristic_relu2 = nn.ReLU()
    ...
    def forward(self, x_char_seq, x_heuristic):
        ...
        #2. Process Heuristic Head
        x_heuristic = x_heuristic.to(device)
        x_heu = self.heuristic_relu1(self.heuristic_fc1(x_heuristic))
        x_heu = self.heuristic_relu2(self.heuristic_fc2(x_heu))
...
def train_epoch(model, dataloader, optimizer, loss_fn, device):
    ...
    for x_char, x_heu, y_batch in dataloader:
        ...
        try:
            ...
            print(f"x_char min: {x_char.min()}, max: {x_char.max()}")
...
if __name__ == "__main__":
    ...
    print(f"Found {N_HEURISTIC_FEATURES} total heuristic features (URL-Lexical + Hyperlink).")
